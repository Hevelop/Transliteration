 ...
 */
class Hevelop_Transliteration_Helper_Data {//extends Mage_Core_Helper_Abstract {
    ...
    /**
     * Alias for $this->transliterate
     * @param $string
     * @param string $unknown
     * @param null $source_langcode
     * @return string
     */
    public function trslt($string, $unknown = '?', $source_langcode = NULL) {
        return $this->transliterate($string, $unknown = '?', $source_langcode = NULL);
    }
    /**
     * Alias for $this->transliterate
     * @param $string
     ...
     */
    protected function _transliteration_replace($ord, $unknown = '?', $langcode = NULL) {
        static $map = array();

        $bank = $ord >> 8;

        if (!isset($map[$bank][$langcode])) {
            $file = __DIR__ . DS . 'data' . DS . sprintf('x%02x', $bank) . '.php';
            if (file_exists($file)) {
                include $file;
